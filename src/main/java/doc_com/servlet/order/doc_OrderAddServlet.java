...
/**
 * Servlet implementation class doc_OrderAddServlet
 */
@WebServlet("/doc_OrderAddServlet")
public class doc_OrderAddServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public doc_OrderAddServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(oid2.equals("")||otime.equals("")||uid2.equals("")||oresult.equals("")||omedicine.equals("")||omoney2.equals("")) {
			request.getSession().setAttribute("sta","创建订单失败，请检查表单是否填写完整");
		}else {
			int oid = Integer.parseInt(oid2);
		int uid = Integer.parseInt(uid2);
		double omoney = Double.parseDouble(omoney2);
		sta = OrderDao.add(oid, otime, uid, oresult, omedicine, omoney);
		}
