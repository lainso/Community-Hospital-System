...
public class restDao {
	//添加请假信息
	public static boolean add(int rid,int did,String rreason,String rstart,String rend,int rcondition) {
...
//修改请假信息
public static boolean chanage(int rid,int did,String rreason,String rstart,String rend,int rcondition) {
	Connection conn = DBUtil.getCon();
	String sql = "UPDATE rest SET did=?,rreason=?,rstart=?,rend=?,rcondition=? where rid=?;";
...
public static boolean delete(int rid) {
	Connection conn = DBUtil.getCon();
	String sql = "DELETE FROM rest WHERE rid=?;";
...
//查询单个请假集合	
	public static ArrayList<Rest> get_List(String mid) {
		ArrayList<Rest> tag_Array = new ArrayList<Rest>();//建立一个数组集合
		Connection conn = DBUtil.getCon();
		String sql = "select * FROM rest where rid=?";
